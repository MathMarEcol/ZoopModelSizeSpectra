...
source("ZooMSS_Param.R")
source("ZooMSS_ODE.R")
source("ZooMSS_Stability.R")
...
out <- ZooMSS_ODE(state = init.dist, parms = params, test = 1, zoo.beta = 0)
...
zoo.0 = sum(ave.size.spectra0[zoop.range]*zoop.weights*params$dx) # Average biomass density of zooplankton
fish.0 = sum(ave.size.spectra0[-zoop.range]*fish.weights*params$dx) # Average biomass density of fish
...
params$alpha.z <- 1.01
params$sigma.z <- 1.75
pbbar <- 1 # Do you want a progress bar
...
for(i in 1:N.runs){
  ...
  if(i <= l.m){
    ...
    out <- ZooMSS_ODE(state = init.dist, parms = params, pbar = pbar)
  ...
  for(j in 1:l.s){
    ...
    for(k in 1:l.K){
      ...
      if(curr.row < 1860 | curr.row > 3783){
        ...
        prod.store[curr.row, 2] = sum(ave.mort.fish*slope.store[curr.row, -zoop.range]*fish.weights*params$dx)
