### Author: Ryan Heneghan and Jason Everett
### Date: May 2019
...
############# ############# ############# ############# ############# 
############# LOAD STATISTICAL MODEL DATA FIRST
############# ############# ############# ############# ############# 
taxa <- c("Omni", "Carn", "Jelly", "Chaet", "Salps", "Euphaus", "Larv")
for (i in 1:1){
  layer <- readRDS(paste0("../../StatisticalModel/ZooplanktonModels/ModelOutput/GlobalLayers/Layer_",sprintf('%02g', i),"_",taxa[i],".rds"))
}

# 1X1 degree grid squares globally
lon <- seq(-179.5,179.5,5)
lat <- seq(-89.5,89.5,5)
lonlat <- expand.grid(Long = lon, Lat = lat)

## Reduce the data to 5 degrees
layer5 <- raster::aggregate(layer, fact=5, fun=median, expand=TRUE, na.rm=TRUE)
dflayer5 <- as.data.frame(rasterToPoints(layer5))



############# ############# ############# ############# ############# 
####### This is just a check so I can plot the data if needed #######
############# ############# ############# ############# ############# 
# source("../../StatisticalModel/ZooplanktonModels/fWorldBordersMoll.R")
...
# data_moll <- projectRaster(layer5, crs = mollCRS, over = TRUE)
...
############# ############# ############# ############# ############# 
############# LOAD NUMERICAL MODEL NOW
############# ############# ############# ############# ############# 

# Ryan
# - How to recreate abundance
# - Did you use temp/chl correctly

# Get lat/lon bins for the 200 location
# enviro_data <- readRDS("../../NumericalModel//Multi-ZooSizeSpectrumModel/store5.RDS") # Run this if you only want 503 observations, instead of full ~1600
enviro_data <- read.csv("../../NumericalModel//Multi-ZooSizeSpectrumModel/enviro_5d_3.csv") # Load in environmental data
...
mtaxa <- c("Flagellates","Ciliates","Larvaceans","Omni Cop",
           "Carn Cop","Euphausiids","Chaetog","Salps",
           "Jellyfish","Fish_Small","Fish_Med","Fish_Large")

load("../../NumericalModel/Multi-ZooSizeSpectrumModel/07AprilModelOutput/res.RData")
