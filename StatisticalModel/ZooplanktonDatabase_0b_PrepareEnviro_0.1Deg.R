...
saveRDS(Bathyagg, file = paste0(EnviroDir,"/Bathy_raster_oneTenthDeg.rds"))
...
for(i in 1:12){
  ...
  names(SSTagg) <- "SST"
  saveRDS(SSTagg, file = paste0(EnviroDir,"/SST_raster_",mnth[i],"_oneTenthDeg.rds"))
  ...
  names(Chlagg) <- "Chl"
  saveRDS(Chlagg, file = paste0(EnviroDir,"/Chl_raster_",mnth[i],"_oneTenthDeg.rds"))
...
saveRDS(SSTbrick_mn, file = paste0(EnviroDir,"/SST_raster_00Mean_oneTenthDeg.rds"))


Chlbrick_mn  <- calc(Chlbrick, fun = mean)
names(Chlbrick_mn) <- "Chl"
saveRDS(Chlbrick_mn, file = paste0(EnviroDir,"/Chl_raster_00Mean_oneTenthDeg.rds"))
